...
'@interval_to_numbers'(<, F0, T) -->
    { integer_add(1, F0, F), compare(O, F, T) },
...
'@interval_to_numbers'(inf-n(T)) -->
    % { throw(error(instantiation_error,_)) }.
    '@interval_to_numbers_inf'(T).
'@interval_to_numbers'(n(F)-sup) -->
    % { throw(error(instantiation_error,_)) }.
...
'@interval_bound'(>, F-n(T), n(B0), n(B)) :-
    (   % N0 cis -(-F div n(B0)), n(0) cis_lt N0,
        foldl(integer_add(-1), B0, B1),
...
'@intervals_positive_expand'(=, A, Is) --> '@intervals_positive_expand'(A, Is).

'@intervals_positive_expand'(_, []) --> [].
'@intervals_positive_expand'(A, [F0-T0|Is]) -->
    { F cis n(A)*F0, T cis n(A)*T0 },
