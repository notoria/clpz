...
parse_reified(E, R, D,
              [g(cyclic_term(E)) => [g(domain_error(clpz_expression, E))],
               ...
               #E            => [g(must_be_fd_integer(E)), g(R=E), g(D=1)],
               m(A+B)        => [d(D), p(iadd(A,B,R)), a(A,B,R)],
               m(A*B)        => [d(D), p(imul(A,B,R)), a(A,B,R)],
               m(A-B)        => [d(D), p(iadd(R,B,A)), a(A,B,R)],
               m(-A)         => [d(D), p(iadd(A,R,0)), a(R)],
               m(max(A,B))   => [d(D), p(pleq(A,R)), p(pleq(B,R)), p(imax(A,B,R)), a(A,B,R)],
               m(min(A,B))   => [d(D), p(pleq(R,A)), p(pleq(R,B)), p(imin(A,B,R)), a(A,B,R)],
               m(abs(A))     => [d(D), g(#R#>=0), p(iabs(A, R)), a(A,R)],
               ...
               m(A mod B)    => [skeleton(A,B,D,R,imod)],
               ...
               m(sign(A))    => [d(D), p(isgn(A, R)), a(A,R)],
               m(A<<B)       => [function(D,<<,A,B,R)],
               m(A>>B)       => [function(D,>>,A,B,R)],
               m(A/\B)       => [function(D,/\,A,B,R)],
               m(A\/B)       => [function(D,\/,A,B,R)],
               m(xor(A, B))  => [skeleton(A,B,D,R,ixor)],
...
make_parse_reified(Clauses) :-
        ...
        list_map(expand_term, Clauses0, Clauses).

parse_reified_clauses(Clauses) :-
        parse_reified(E, R, D, Matchers),
        list_map(parse_reified(E, R, D), Matchers, Clauses).
...
parse_reified(E, R, D, Matcher, Clause) :-
        ...
        Clause = (parse_reified_clpz(Head, R, D) --> Body).
...
reified_goal(p(Vs,C), _) -->
        [{propagator_from_constraint(C, P)}],
        parse_init_dcg(Vs, P),
        [{queue_unify(Vs),propagator_trigger(P, [])}],
        [( { propagator_dead(P) } -> [] ; [p(P)])].
reified_goal(p(C), Ds) -->
        { term_variables(C, Vs) },
        reified_goal(p(Vs,C), Ds).
...
reified_goal(skeleton(A,B,D,R,F), Ds) -->
        { C =.. [F,X,Y,Z] },
        reified_goals([d(D1),l(p(P)),g(propagator_from_constraint(C, P)),
...
parse_init_dcg([], _)     --> [].
parse_init_dcg([V|Vs], P) --> [{propagator_variable(P, V)}], parse_init_dcg(Vs, P).

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
?- use_module(library(lists)),
   use_module(library(format)),
   clpz:parse_reified_clauses(Cs),
   list_map(portray_clause, Cs).
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
